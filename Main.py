...
def run_backtest_for_market(market_name: str, csv_path: str, risk_per_trade: float):
    print("\n" + "="*70)
    print(f" BACKTEST FÖR MARKNAD: {market_name} (risk {risk_per_trade*100:.2f}% per trade)")
    print("="*70 + "\n")
...
for m in markets:
    try:
        stats, trades_df = run_backtest_for_market(
            m["name"],
            m["csv"],
            m["risk_per_trade"],      # ⬅️ skicka in risk per marknad
        )
